...
const { selectedProduct, prewievProducts, akciosarSzazalek, modalStatus, message, allSelected, cimkekForShow, getProdData, addNewTag, selectedProd, selectedIndex, changed, reload, differentObjects } = storeToRefs(TagsStore())
const { updateTag, changeFontFamilytag, changeFontStyletag,  changeFontSize, pushToSelectedProducts, reducePrice, receiveEmit, fetchCimkek, deleteTag, TagProd, setInd, setDataToSelectedProducts, changeSelectedProd, emptingVariables } = TagsStore()
fetchCimkek()
                ...
                <div class="container" :class="{hideElem : selectedIndex !== null && selectedIndex !== index}">
                    <div class="row">
                        <div class="col-6">
                            ...
                            <p class="d-flex align-items-start justify-content-start text-muted fw-light fs-6">{{item.szoveg}} </p> 
                        </div>
                        <div class="col">
                        ...
                        <div class="col buttons align-items-end pt-4 ms-5 ">
                            ...
                            <button v-if="item.edit" type="button" class="btn secoundaryBtna btn-lg ms-2" @click="item.edit = !item.edit,  emptingVariables(), differentObjects = []" data-bs-toggle="tooltip" data-bs-placement="top" title="Bezár" ><font-awesome-icon :icon="['fas', 'xmark']" /></button>
                ...
                <div class="" v-if="item.edit">
                    <div class="container mb-4 pt-4 back mt-4">
                        <div class="col-12 mb-8">
                                <ul class="m-4">
                                    <ol>
                                    <form action="{{ route('updateCimke', $post) }}" method="POST" @submit.prevent="updateTag(item.cimkeId, item.cim, item.szoveg, item.hatterszin, item.betuszin, item.betutipus, item.betustilus, item.betumeret, item.akciosarFt, item.akciosarSzazalek)" id="updateTag">
                                        <div class="row">
                                            <div class="col-3">
                                            ...
                                            <div class="row mt-4">
                                                ...
                                                <p class="text-danger fs-6 text" v-if="item.akciosarFt || item.akciosarSzazalek">Figyelem! Ha már valamelyik terméknél korábban megadtál egy akciós árat, akkor ezzel felülírod!</p>
                                        </div>
                                                <div class="align-items-end mt-4">
                                                    <button type="submit" class="btn secoundaryBtna px-2 mb-4 ms-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Mentés" > <font-awesome-icon :icon="['fas', 'floppy-disk']"/> Mentés</button>
