  ...
  <div class="container fontcolor cim">
    <div class="justify-content-center text-center p-2">
      <div class="divider fontcolor"><span></span><span><font-awesome-icon :icon="['fas', 'cart-shopping']" />
        ...
        <div v-if="userData.orders.length > 0">
      ...
      <div class="container mb-4 pt-4">
        <ul class="list">
          <li class="product-li row row-cols-7 mt-2 p-4" v-for="order in userData.orders" :key="order.id">
            ...
            <div class="col">
              <button type="button" class="btn secoundaryBtna btn-lg ms-2"
                :class="{ rotateBtn: isOrderVisible(order.id) }" @click="toggleProduct(order.id)"
            ...
            <div v-if="isOrderVisible(order.id)">
              <h5 class="text-uppercase fw-bold"><font-awesome-icon :icon="['fas', 'cart-shopping']" /> Rendelt termékek
              </h5>
              ...
              <div class="row mt-2 flex-fill">
                <ul class="listProduct">
                  ...
                  <li class="row mt-2 p-2 align-items-center">
                    <div class="col">
                      <p class="">{{ order.vasarlo.nev }}</p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szamlazasi_cim.iranyitoszam }} </p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szamlazasi_cim.telepules }}</p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szamlazasi_cim.utca }}</p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szamlazasi_cim.hazszam }}</p>
                    </div>
                    <hr class="" />
                  </li>
                </ul>
              ...
              <div v-else class="row mt-2 flex-fill">
                <ul class="listProduct">
                  ...
                  <li class="row mt-2 p-2 align-items-center">
                    <div class="col">
                      <p class="">{{ order.szallitasi_cim.iranyitoszam }} </p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szallitasi_cim.telepules }}</p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szallitasi_cim.utca }}</p>
                    </div>
                    <div class="col">
                      <p class="">{{ order.szallitasi_cim.hazszam }}</p>
                    </div>
                    <hr class="" />
                  </li>
                </ul>
    ...
    <div v-else class="text-center">
      <p>Ehhez a felhasználóhoz még nem rögzítettünk rendelést.</p>
      <p>Látogasson el webshopunkba</p>
