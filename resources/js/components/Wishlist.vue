...
const { update, editStore, fetchWishlist, removeFromWishList, receiveEmit, deleteWish } = ProductStore()
fetchWishlist();
...
<div class="d-flex flex-column justify-content-center pt-md-8">
  <div class="divider fontcolor"><span></span><span><font-awesome-icon :icon="['fas', 'star']" />
    KÍVÁNSÁGLISTÁM </span><span></span></div>
    </div>
    <loader v-if="loading"></loader>
    <div class="container-fluid">
        <div class="row justify-content-center g-0">
            <div v-if="emptyMessage" class="justify-content-center text-center" >Jelenleg üres a kívánságlista. <a href="termekek">Adj hozzá csodálatos rózsákat!</a></div>
            <div class="col-lg-3 col-md-4 col-sm-5 col-8 m-2 product position-relative" v-for="wish in wishlist" key="wish.id" id="box">
                <a href="#" class="row">
                    <div class="termek-kep">
                        <img :src="wish.product[0].img" class="img-fluid img-maxDefault">
                        <div v-if="wish.product[0].keszlet <= 0" class="elfogyott px-2 text-start">Elfogyott</div>
                        <div v-else class="keszleten px-2 text-start">Készleten</div>
                    </div>
                </a>
                <div class="row justify-content-center text-center m-1">
                    <div class="szin">{{ wish.product[0].szin }}</div>
                      <div class="nev">{{ wish.product[0].nevHu }}</div>
                      <div class="row justify-content-center">
                            <div class="col-sm-8 col-md-8 col-lg-8"><p class="ar">{{wish.product[0].ar }} ,-Ft</p></div>
                        </div>
                      <div class="col-md-2 col-lg-col-2 col-xl-2 imagebuttondiv wishlist">
                          <img src="../../assets/kepek/heartfull.png" class="heartIconWish"  alt="rolunk2" @click="removeFromWishList(wish.product[0].id)">
                      </div>
                    <button v-if="wish.product[0].keszlet > 0" class="col-md-8 col-lg-10 col-xl-10 kosarba kosarba-active text-center" data-bs-toggle="modal" data-bs-target="#KosarbaModal" v-on:click="toModal(wish)">
